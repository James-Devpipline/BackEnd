...
def add_personal_record(req: Request):
    ...
    new_pr = PersonalRecords.new_personal_record()

    populate_object(new_pr, req_data)
...
def get_all_personal_records(req: Request):
    personal_records = db.session.query(PersonalRecords).all()
...
def get_personal_record(req: Request, ex_id):
    personal_record = db.session.query(PersonalRecords).filter(PersonalRecords.exercise_id == ex_id).first()
...
def update_personal_record(req: Request, ex_id):
...
def delete_personal_record(req: Request, ex_id):
    personal_record = db.session.query(PersonalRecords).filter(PersonalRecords.exercise_id == ex_id).first()
